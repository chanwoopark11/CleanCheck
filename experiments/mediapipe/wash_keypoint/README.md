...
#### **v5: 손 좌표 겹침 해결**
* **설명**: 손 씻기 메커니즘을 3개의 시나리오로 분류하고, 각 상황에 맞는 접촉면을 판단하여 세척 정보를 제공합니다. 특히, 두 손이 너무 가까워지거나 가려질 때 오작동을 방지하여 보다 정확하고 신뢰성 있는 손 씻기 인식을 목표로 합니다.
* **주요 기능**
   * **두 손 간 거리 기반 세척 인식 중단:**
      * 목표:  두 손이 너무 가까이 붙어 겹치거나 가려져 정확한 랜드마크 감지가 어려운 경우 (옥클루전), 또는 '손이 몰렸다'고 판단되는 경우 오작동을 방지합니다.
      * 방식: 양 손목 랜드마크(0번) 간의 3차원 거리를 계산하여, 이 거리가 미리 정의된 MIN_HAND_DISTANCE_THRESHOLD 값보다 작아지면, 현재 프레임의 모든 세척 인식을 즉시 중단하고 타이머를 초기화합니다.
      * 효과: 불확실한 상황에서 '닦였다'고 잘못 인식하는 것을 방지하여 시스템의 신뢰도를 크게 향상시켰습니다. (이 값은 사용자의 환경에 따라 조정 가능하며, 기본 80으로 설정되었습니다.)
